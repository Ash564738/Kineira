# api/routers/data_collection.py
import logging
import os
...
async def start_collection(
    action: str,
    video_num: int,
    overwrite: Optional[bool] = Query(False, description="Overwrite existing video")
):
    if action not in ACTIONS:
        raise HTTPException(status_code=400, detail=f"Unknown action: {action}")
    ...
    success = collector.start_video_collection(action, video_num, overwrite=overwrite)
    if not success:
...
async def save_frame_vector_batch(action: str, video_num: int, payload: BatchFramePayload):
    ...
    for item in payload.frames:
        ...
        if len(item.keypoints) != FEATURE_SIZE:
            raise HTTPException(status_code=400, detail=f"Frame {item.frame_num}: expected {FEATURE_SIZE} values, got {len(item.keypoints)}")
